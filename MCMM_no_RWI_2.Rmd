...
lay_vol              = array(0, dim=c(8,1,N)) # array of layer volumes
  for(k in 1:N){
    ...
    if(RWI_on == TRUE){
      ...
      RWI_cont[,1,k] = abs(b) * RWI # add RWI cont
    }
    # TDS[,1,k] = layer_depths # inital TDS-depth profile
    # RWI_cont[,1,k] = abs(b) * RWI # add RWI cont
    ...
    for(j in 1:7){
      ...
      # baseline correction from rock water interactions when RWI not present
      ...
      if(RWI_on == FALSE){
        RWI_cont[j,1,k] <- mean_bl # save RWI for non-RWI case
      }
      
      if(RWI_on == TRUE){
        TDS[,j+1,k] <- TDS[,j+1,k] + RWI_cont[,,k] # save RWI for non-RWI case
      }
...
df2$dc <- 1:7
filter(df2, dc == 1, tc == 1) %>% summarise(md = mean(d), mtds = median(tds))
filter(df2, dc == 1, tc == 2) %>% summarise(md = mean(d), mtds = median(tds))
```
...
#p3 <- ggplot(filter(df4_rwi, t %in% c(0,50,100,150)), aes(factor(t), m/1000000, fill = type)) + # convert m from tons to Mtons
  ...
  labs(x = "", y = "Annual mass (Metric Mtons)") +
  facet_wrap(~class, ncol = 2) +
